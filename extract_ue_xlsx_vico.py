...
def extract_ues_from_excel(excel_path, output_json, output_ttl=None):
    ...
    for i in range(start_row, len(df)):
        ...
        # Nouvelle fiche si la ligne précédente est un code seul suivi d'un champ connu
        if previous_row_values and len(previous_row_values) == 1 and row_values[0].strip() in champ_to_predicate:
            if current_ue:
                ues.append(current_ue)
            current_ue = {"parcours_level": parcours_level, "parcours_code": parcours_code}
            current_ue["code"] = previous_row_values[0].strip()
    ...
    if output_ttl:
        ...
        for ue in ues:
            ...
            subj = URIRef(f"{EX}UE_{quote(code)}") if re.match(r'^[XY][A-Z0-9]{6}$', code) else URIRef(f"{EX}UE_NO_CODE_{quote(code)[:20].replace('%', '')}")
