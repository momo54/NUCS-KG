...
#        print(f"Processing row {i}: '{normalize(first_col)}': in keys ?: {normalize(first_col)  in raw_metadata_fields.keys()}' -> '{second_col}' (next: '{next_first_col}')")
        if code_regex.match(first_col.strip()) or (normalize(first_col) not in raw_metadata_fields.keys() and normalize(next_first_col) 
                in ["lieu d'enseignement", "langue d'enseignement"]):
#            print(f"Found new UE code: '{first_col}'")
            if current_ue:
